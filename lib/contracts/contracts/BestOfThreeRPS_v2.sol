...
///         v2 adds openSeriesIds tracking for frontend discoverability and
///         RevealDeadlineSet events for off-chain monitoring.
contract BestOfThreeRPS {
    ...
    // ── Open Series Tracking (v2) ────────────────────────────────────────────
    uint256[] private openSeriesIds;
    mapping(uint256 => uint256) private openSeriesIndex;
    ...
    event RevealDeadlineSet(uint256 indexed seriesId, uint8 round, uint256 deadline);  // v2: new event
    ...
    function createSeries(bytes32 commitment) external payable returns (uint256 id) {
        ...
        // v2: Track as open
        openSeriesIndex[id] = openSeriesIds.length;
        openSeriesIds.push(id);
        isOpen[id] = true;
    ...
    function joinSeries(uint256 id, bytes32 commitment) external payable {
        ...
        // v2: Remove from open series
        _removeOpenSeries(id);
        
        uint256 deadline = uint256(s.roundStartedAt) + REVEAL_TIMEOUT;
        emit SeriesJoined(id, msg.sender);
        emit RevealDeadlineSet(id, 1, deadline);  // v2: new event
    }
    ...
    function commitRound(uint256 id, bytes32 commitment) external {
        ...
        if (s.committed1 && s.committed2) {
            ...
            s.roundStartedAt = uint64(block.timestamp);
            uint256 deadline = uint256(s.roundStartedAt) + REVEAL_TIMEOUT;
            emit RevealDeadlineSet(id, s.currentRound, deadline);  // v2: new event
        }
    }
    ...
    /// @notice Get all open series IDs. Frontend can iterate to show available games.
    function getOpenSeries() external view returns (uint256[] memory) {
        return openSeriesIds;
    }

    /// @notice Get count of open series.
    function getOpenSeriesCount() external view returns (uint256) {
        return openSeriesIds.length;
    }

    /// @notice Get paginated open series (start index, limit).
    function getOpenSeriesPaginated(uint256 start, uint256 limit) external view returns (uint256[] memory) {
        if (start >= openSeriesIds.length) {
            return new uint256[](0);
        }
        uint256 end = start + limit;
        if (end > openSeriesIds.length) {
            end = openSeriesIds.length;
        }
        uint256[] memory result = new uint256[](end - start);
        for (uint256 i = start; i < end; i++) {
            result[i - start] = openSeriesIds[i];
        }
        return result;
    }
    ...
    /// @notice Remove a series from the openSeriesIds array.
    function _removeOpenSeries(uint256 id) internal {
        if (!isOpen[id]) return;
        
        uint256 index = openSeriesIndex[id];
        uint256 lastId = openSeriesIds[openSeriesIds.length - 1];
        
        openSeriesIds[index] = lastId;
        openSeriesIndex[lastId] = index;
        openSeriesIds.pop();
